  ...
  const Header = (
    <thead>
      ...
      <tr className="bg-gray-50 text-xs">
        {visible.title && <th className="px-4 py-3  border-r border-gray-300  "><input value={filters.title} onChange={e => { setFilters(f => ({ ...f, title: e.target.value })); setPage(1); }} placeholder="Search..." className="w-full px-3 py-1.5 text-xs border border-gray-300 rounded" /></th>}
        {visible.manufacturer && <th className="px-4 py-3  border-r border-gray-300  "><input value={filters.manufacturer} onChange={e => { setFilters(f => ({ ...f, manufacturer: e.target.value })); setPage(1); }} placeholder="Search..." className="w-full px-3 py-1.5 text-xs border border-gray-300 rounded" /></th>}
        {visible.category && <th className="px-4 py-3  border-r border-gray-300  "><input value={filters.category} onChange={e => { setFilters(f => ({ ...f, category: e.target.value })); setPage(1); }} placeholder="Search..." className="w-full px-3 py-1.5 text-xs border border-gray-300 rounded" /></th>}
        {visible.merchant && <th className="px-4 py-3  border-r border-gray-300  "><input value={filters.merchant} onChange={e => { setFilters(f => ({ ...f, merchant: e.target.value })); setPage(1); }} placeholder="Search..." className="w-full px-3 py-1.5 text-xs border border-gray-300 rounded" /></th>}
        {visible.inStock && <th className="px-4 py-3  border-r border-gray-300  "><input value={filters.inStock} onChange={e => { setFilters(f => ({ ...f, inStock: e.target.value })); setPage(1); }} placeholder="Search..." className="w-full px-3 py-1.5 text-xs border border-gray-300 rounded" /></th>}
        {visible.price && <th className="px-4 py-3  border-r border-gray-300  "><input value={filters.price} onChange={e => { setFilters(f => ({ ...f, price: e.target.value })); setPage(1); }} placeholder="Search..." className="w-full px-3 py-1.5 text-xs border border-gray-300 rounded" /></th>}
        <th></th>
      </tr>
    </thead>
  );
  ...
  return (
    <div className="bg-white rounded-xl shadow-xl border border-gray-200 p-6 max-w-full">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {/* <Link href="/super-admin/products/new" className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-lg shadow-md transition flex items-center gap-2">
            ...
            {loading ? (
              Array.from({ length: 8 }).map((_, i) => (
                <tr key={i} className="animate-pulse">
                  {visibleKeys.map(k => (
                    <td key={k} className="px-4 py-3  border-r border-gray-300  "><div className="h-4 bg-gray-200 rounded"></div></td>
                  ))}
              ...
              pageData.map(r => (
                <tr key={r.id} className="hover:bg-gray-50 transition-colors duration-150">
                  ...
                  {visible.manufacturer && <td className="px-4 py-3  border-r border-gray-300 text-sm text-gray-700">{sanitize(r.manufacturer)}</td>}
                  {visible.category && <td className="px-4 py-3  border-r border-gray-300  text-sm text-gray-700">{r.category?.name || "N/A"}</td>}
                  {visible.merchant && <td className="px-4 py-3  border-r border-gray-300 text-sm text-gray-700">{r.merchant?.name || "N/A"}</td>}
                  {visible.inStock && (
                    <td className="px-4 py-3  border-r border-gray-300  text-sm">
                  ...
                  {visible.price && <td className="px-4 py-3  border-r border-gray-300   text-sm font-semibold">₹{r.price.toLocaleString()}</td>}
                  <td className="px-4 py-3  border-r border-gray-300  text-sm">
                    <Link href={`/super-admin/products/${r.id}`} className="text-blue-600 hover:text-blue-800 hover:underline font-medium">
                      View Details
                    </Link>
                  </td>
                </tr>
