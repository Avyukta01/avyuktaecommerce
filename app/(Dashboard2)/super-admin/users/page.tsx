...
export default function SuperAdminUsersPage() {
  ...
  const processed = useMemo(() => {
    ...
    d = d.filter((r) => {
      ...
      const date = new Date(r.createdAt).toLocaleDateString().toLowerCase().includes(filters.createdAt.toLowerCase());
  ...
  const visibleKeys = (["email", "role", "createdAt"] as ColKey[]).filter((k) => visible[k]);
      ...
      .map((r) =>
        visibleKeys
          .map((k) => {
            ...
            if (k === "createdAt") return new Date(r.createdAt).toLocaleDateString();
            return (r as any)[k];
          })
          .join("\t")
      )
  ...
  const doPrint = () => {
    ...
    const body = processed
      ...
      <tr>
        ...
        <td><a href="/super-admin/users/${r.id}">View & Edit</a></td>
      </tr>`
      )
      .join("");
    w.document.write(`
      ...
      </head><body><h1>Users Management</h1>
    ...
    <thead>
      <tr className="bg-gray-50 border-b-2 border-gray-300 text-gray-900 text-xs font-bold uppercase tracking-wider">
        ...
        {visible.createdAt && (
          ...
          >
            <div className="flex items-center gap-1">Created At {arrow("createdAt")}</div>
          </th>
        )}
