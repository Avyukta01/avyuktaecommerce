        ...
        >
          <div className="max-w-[550px] w-full">
            ...
            <a
              ...
              className="inline-flex font-medium text-white bg-blue-600 py-3 px-10 rounded-md hover:bg-blue-700 mt-6 transition"
            >
              {banners[0].ctaText}
            </a>
          </div>
            ...
            >
              <Image
                ...
                className={`absolute top-1/2 -translate-y-1/2 ${
                  banner.textAlign === "left" ? "right-4 sm:right-10" : "left-4 sm:left-10"
                } -z-1`}
              />

              <div className={banner.textAlign === "left" ? "text-left" : "text-right"}>
                <span className="block text-lg text-black mb-2">{banner.title}</span>
                <h2 className="font-bold text-xl lg:text-2xl text-black mb-2">
                  {banner.description}
                </h2>
                {banner.discount && (
                  <p className="font-semibold text-black/70 mb-3">{banner.discount}</p>
                )}
                <a
                  href={banner.ctaLink}
