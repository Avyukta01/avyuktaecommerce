...
import { Heart, ShoppingCart, Eye, Ban } from "lucide-react";
...
const ProductItem = ({
  ...
  return (
    <div
      className={`group flex flex-col h-full bg-white shadow-sm transition-all duration-300 transform rounded-2xl ${
        isInStock
    ...
    >
      ...
      {!isInStock && (
        <div className="absolute inset-0 bg-black/40 flex flex-col justify-center items-center text-white rounded-2xl z-10">
          <Ban size={40} className="mb-2 opacity-80" />
          <span className="font-bold text-lg tracking-wide">OUT OF STOCK</span>
        </div>
      )}
        ...
        <Link href={`/product/${product.slug}`}>
          <Image
            ...
            className={`object-contain transition-transform duration-500 ${
              isInStock ? "group-hover:scale-105" : "opacity-60"
            }`}
